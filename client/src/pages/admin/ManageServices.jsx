...
const AdminAdvancedSkinCare = () => {
  ...
  const [newTreatment, setNewTreatment] = useState({ name: "", duration: "", price: "", description: "" });
  const [editingTreatment, setEditingTreatment] = useState(null);

  const handleAddTreatment = () => {
    if (!newTreatment.name || !newTreatment.duration || !newTreatment.price || !newTreatment.description) {
          ...
          <div key={treatment.id} className="treatment-card">
            {editingTreatment && editingTreatment.id === treatment.id ? (
              <>
                ...
                <button onClick={() => setEditingTreatment(null)}>Cancel</button>
              </>
            ) : (
              <>
                ...
                <button onClick={() => handleEditTreatment(treatment)}>Edit</button>
                <button onClick={() => handleDeleteTreatment(treatment.id)}>Delete</button>
              </>
            )}
          </div>
        ))}
